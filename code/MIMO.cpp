...
const std::vector<std::vector<std::complex<double>>>& channel_matrix, double snr_db = 0){
    ...
    for(int i = 0; i < size / 2; ++i){
        ...
        modulation_mimo[2 * i]  = x0 * channel_matrix[0][0] + x1 * channel_matrix[0][1] + noise0;

        modulation_mimo[2 * i + 1] = x0  * channel_matrix[1][0] + x1 * channel_matrix[1][1] + noise1;
...
const std::vector<std::vector<std::complex<double>>>& channel_matrix){
    ...
    std::complex<double> det = channel_matrix[0][0] * channel_matrix[1][1] - channel_matrix[0][1] * channel_matrix[1][0];

    std::complex<double> y0, y1;

    std::vector<std::vector<std::complex<double>>> channel_matrix_inv(2, std::vector<std::complex<double>>(2));

    channel_matrix_inv[0][0] = channel_matrix[1][1] / det;
    channel_matrix_inv[0][1] = -channel_matrix[0][1] / det;
    channel_matrix_inv[1][0] = -channel_matrix[1][0] / det;
    channel_matrix_inv[1][1] = channel_matrix[0][0] / det;
     ...
     for(int i = 0; i < size / 2; ++i){
        ...
        demodulation_mimo[2 * i] = y0 * channel_matrix_inv[0][0] + y1 * channel_matrix_inv[0][1];
        demodulation_mimo[2 * i + 1] = y0  * channel_matrix_inv[1][0] + y1 * channel_matrix_inv[1][1];
